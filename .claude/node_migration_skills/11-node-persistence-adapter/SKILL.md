...
## Purpose
Guide a model through the node persistence adapter part of migrating the confirmed Rust modular library demo to idiomatic Node / TypeScript without changing behavior or architectural boundaries.
...
For **Node Persistence Adapter**, preserve the exact confirmed Rust names and behavior where this skill touches them. If this skill needs details not listed above, inspect the repository again and add only confirmed facts.
